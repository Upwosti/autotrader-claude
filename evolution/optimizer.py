...
class Optimizer:
    ...
    def evolve(
        ...
        for i in range(1, max_iterations + 1):
            ...
            # Step 1: get mutated params
            new_params, param_name, old_val, new_val, _ = self.evolver.next_iteration(
                current_win_rate=self.current_result.win_rate,
                current_trade_count=self.current_result.total_trades,
            )

            # Step 2: run backtest with new params
            new_result = self.run_iteration(new_params, pair=pairs[0])

            # Step 3: evaluate AFTER seeing the new result
            kept = self.evolver.accept(
                new_win_rate=new_result.win_rate,
                old_win_rate=self.current_result.win_rate,
                new_params=new_params,
                new_trade_count=new_result.total_trades,
                min_trades=10,
                new_profit_factor=new_result.profit_factor,
                old_profit_factor=self.current_result.profit_factor,
                new_max_dd=new_result.max_drawdown_pct,
                old_max_dd=self.current_result.max_drawdown_pct,
            )
            ...
            # Send rich evolution update every 5 iterations
            if i % 5 == 0:
                self.telegram.send_evolution_update(
                    iteration=i, param=param_name,
                    old_val=old_val, new_val=new_val,
                    wr_before=self.current_result.win_rate,
                    wr_after=new_result.win_rate, kept=kept,
                )
    ...
    def _send_alert(self, alert_type: str, subject: str, result: BacktestResult):
        ...
        # Use rich methods where available, plain send for generic alerts
        if alert_type in ("new_best", "milestone_100", "milestone_1000", "final_report",
                           "iteration_complete"):
            self.telegram.send_performance_report(stats)
            self.email.send_performance_report(stats)
        else:
            body = (f"Version: v{result.strategy_version} | "
                    f"WR: {result.win_rate:.1%} | RRR: {result.avg_rrr:.2f} | "
                    f"DD: {result.max_drawdown_pct:.2f}% | Trades: {result.total_trades}")
